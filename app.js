...
function efectoHabilidades2(){
    var habilidades = document.getElementById("habilidades2");
    var distancia_skills = window.innerHeight - habilidades.getBoundingClientRect().top;
    if(distancia_skills>=300 && entro==false){
        entro = true;
        const intervalprojectmanager = setInterval(function(){
            pintarBarra(projectmanager, 16, 0, intervalprojectmanager);
        },100);
        const intervaluml = setInterval(function(){
            pintarBarra(uml, 11, 1, intervaluml);
        },100);
        const intervalkanban = setInterval(function(){
            pintarBarra(kanban, 11, 2, intervalkanban);
        },100);
        const intervalscrum = setInterval(function(){
            pintarBarra(scrum, 15, 3, intervalscrum);
        },100);
    }
}
